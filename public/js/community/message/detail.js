...
$(() => {
  ...
  $.ajax({
    url: '/api/communities/message/details',
    type: 'GET',
    data: { communityId, messageId },
    success: (result) => {
      const data = result.data
      console.log(data)
      $('.txt h3').text(`${data.title}`)
      ...
      if (data.titles.next) {
        ...
        $('.next .post_title').html(`<a href="/community/message/detail/${communityId}/${Number(index)+1}/${data.titles.next.id}">${data.titles.next.title}</a>`)
      ...
      if (data.titles.prev) {
        ...
        $('.prev .post_title').html(`<a href="/community/message/detail/${communityId}/${Number(index)-1}/${data.titles.prev.id}">${data.titles.prev.title}</a>`)
  ...
  $('.list_btn').on('click', function () {
    document.location.href = `/community/message/list/${communityId}`
  })
})
